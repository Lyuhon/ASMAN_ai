...
export default function DashboardStats({ user, tg }) {
    ...
    const texts = {
        ru: {
            ...
            noData: 'Нет данных',
            noDataDescription: 'Создайте первый отчет, чтобы увидеть статистику'
        },
        uz: {
            statistics: 'Statistika',
            totalReports: 'Jami hisobotlar',
            ...
            refreshStats: 'Statistikani yangilash',
            noData: 'Ma\'lumot yo\'q',
            noDataDescription: 'Statistikani ko\'rish uchun birinchi hisobotni yarating'
        },
        en: {
            statistics: 'Statistics',
            totalReports: 'Total Reports',
            ...
            noData: 'No Data',
            noDataDescription: 'Create your first report to see statistics'
        }
    }
    ...
    const fetchDashboardStats = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setStatsError(currentTexts.authRequired)
            } else {
                setStatsError(currentTexts.serverError)
            }
        ...
        <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
                ...
                <button
                    ...
                    disabled={statsLoading}
                    className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center hover:bg-gray-200 transition-colors disabled:opacity-50"
                    title={currentTexts.refreshStats}
                >
                    <svg
                        className={`w-4 h-4 text-gray-600 ${statsLoading ? 'animate-spin' : ''}`}
                        fill="none"
            ...
            {statsLoading ? (
                ...
                <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">{currentTexts.noData}</h3>
                    <p className="text-gray-600">{currentTexts.noDataDescription}</p>
                </div>
            )}
        </div>
    )
