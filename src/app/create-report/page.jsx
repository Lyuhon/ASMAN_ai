...
// app/create-report/page.js - С НОВОЙ СИСТЕМОЙ ПЕРЕВОДОВ
'use client'
import { useState } from 'react'
import { useRouter } from 'next/navigation'
import Link from 'next/link'
...
import { getTranslations, interpolateString } from '../utils/translations'
...
export default function CreateReportPage() {
    ...
    // Получаем переводы
    const t = getTranslations('createReport', getUserLanguage())
    ...
    const reportTypes = [
        ...
        {
            id: 'eyecsite',
            title: t.eyecsiteTitle,
            description: t.eyecsiteDescription,
            icon: (
                <svg className="w-8 h-8 text-emerald-800" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
    ...
    const getStepsForType = (type) => {
        if (type === 'qogi') {
            return [
                { id: 1, title: t.opgalImage, desc: t.opgalImageDesc, field: 'image', fileType: 'image', accept: 'image/*', maxSize: '5 МБ', optional: false },
                { id: 2, title: t.firstVideo, desc: t.firstVideoDesc, field: 'video1', fileType: 'video', accept: '.ts', maxSize: '100 МБ', optional: false },
                { id: 3, title: t.secondVideo, desc: t.secondVideoDesc, field: 'video2', fileType: 'video', accept: '.ts', maxSize: '100 МБ', optional: false },
                { id: 4, title: t.csvData, desc: t.csvDataDesc, field: 'data_file', fileType: 'data', accept: '.csv', maxSize: '10 МБ', optional: false },
                { id: 5, title: t.reportSettings, desc: t.reportSettingsDesc },
                { id: 6, title: t.reportGeneration, desc: t.reportGenerationDesc }
            ]
        } else if (type === 'eyecsite') {
            return [
                { id: 1, title: t.imageFile, desc: t.imageFileDesc, field: 'image', fileType: 'image', accept: 'image/*', maxSize: '5 МБ', optional: false },
                { id: 2, title: t.videoFile, desc: t.videoFileDesc, field: 'video', fileType: 'video', accept: '.ts', maxSize: '100 МБ', optional: false },
                { id: 3, title: t.wordDocument, desc: t.wordDocumentDesc, field: 'docx', fileType: 'docx', accept: '.docx', maxSize: '20 МБ', optional: false },
                { id: 4, title: t.excelTable, desc: t.excelTableDesc, field: 'data_file', fileType: 'data', accept: '.xlsx', maxSize: '50 МБ', optional: false },
                { id: 5, title: t.reportSettings, desc: t.reportSettingsDesc },
                { id: 6, title: t.reportGeneration, desc: t.reportGenerationDesc }
            ]
        }
    ...
    const validateFile = (file, type) => {
        ...
        switch (type) {
            ...
            case 'docx':
                if (!file.name.toLowerCase().endsWith('.docx')) {
                    alert(t.docxMustBeDocx)
                    return false
                }
                if (file.size > maxDocxSize) {
                    alert(t.docxSizeLimit)
                    return false
                }
                break
            ...
            case 'xlsx':
                if (!file.name.toLowerCase().endsWith('.xlsx')) {
                    alert(t.xlsxMustBeXlsx)
                    return false
                }
                if (file.size > maxXlsxSize) {
                    alert(t.xlsxSizeLimit)
                    return false
                }
                break
            ...
            case 'data':
                ...
                if (isQogi && !file.name.toLowerCase().endsWith('.csv')) {
                    alert(t.qogiNeedsCsv)
                    return false
                }
                if (isEyecsite && !file.name.toLowerCase().endsWith('.xlsx')) {
                    alert(t.eyecsiteNeedsXlsx)
                    return false
                }
                break
        }
    ...
    const generateReport = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                setNotification({
                    type: 'success',
                    title: t.reportCreatedSuccessfully,
                    message: interpolateString(t.reportCreated, {
                ...
                if (result.error === 'uncertainty_too_high') {
                    setNotification({
                        type: 'error',
                        title: t.reportRejected,
                        message: interpolateString(t.uncertaintyTooHigh, {
                            percent: result.data?.uncertainty_percentage
                        }),
                        action: () => {
                            setNotification(null)
                            // Возвращаем на начало
            ...
            <div className="p-6 pb-24">
                <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                    <div className="flex items-center">
                        ...
                        <span className="text-red-800 text-sm">{t.telegramAuthRequired}</span>
                    </div>
                </div>
            </div>
        )
    ...
    return (
        <div className="p-6 pb-24">
            ...
            <div className="flex items-center justify-between mb-8">
                ...
                <h1 className="text-xl font-semibold text-gray-900">
                    {currentStep === 0 ? t.selectReportType : t.createReport}
                </h1>
                <div className="w-10"></div>
            </div>
            ...
            {currentStep > 0 && (
                <>
                    ...
                    <div className="mb-8">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-medium text-emerald-800">
                                {interpolateString(t.stepOf, { step: currentStep, total: totalSteps })}
                            </span>
                            <div className="flex items-center gap-2">
                                <span className="text-xs text-gray-600">
                                    {reportTypes.find(type => type.id === selectedType)?.title}
                                </span>
                                <span className="text-sm text-gray-600">
                                    {Math.round((currentStep / totalSteps) * 100)}%
                                </span>
                            ...
                            <div className="text-center">
                                {isGenerating ? (
                                    <div className="py-4">
                                        <div className="w-16 h-16 border-4 border-emerald-200 border-t-emerald-800 rounded-full animate-spin mx-auto mb-4"></div>
                                        <h3 className="text-lg font-semibold text-gray-900 mb-2">
                                            {uploadProgress.isUploading ? t.uploadingFiles : t.generatingReport}
                                        </h3>
                                        ...
                                        <p className="text-gray-600">
                                            {uploadProgress.isUploading ? t.uploading : t.processing}
                                        </p>
                                    </div>
                                ) : (
                                    <div className="py-4">
                                        ...
                                        <h3 className="text-lg font-semibold text-gray-900 mb-2">
                                            {t.readyToGenerate}<br />{t.confirmCreation}
                                        </h3>
                                        <div className="bg-gray-50 rounded-xl p-4 text-left">
                                            <div className="space-y-2 text-sm">
                                                <div className="flex justify-between">
                                                    <span className="text-gray-600">{t.reportType}:</span>
                                                    <span className="font-medium">{selectedType?.toUpperCase()}</span>
                                                </div>
                                                {formData.data_file && (
                                                    <div className="flex justify-between">
                                                        <span className="text-gray-600">{t.dataFile}:</span>
                                                        <span className="font-medium">{formData.data_file?.name}</span>
                                                    </div>
                                                )}
                                                {formData.image && (
                                                ...
                                                {formData.video1 && (
                                                    <div className="flex justify-between">
                                                        <span className="text-gray-600">{t.video1}:</span>
                                                        <span className="font-medium">{t.uploaded}</span>
                                                    </div>
                                                )}
                                                {formData.video2 && (
                                                    <div className="flex justify-between">
                                                        <span className="text-gray-600">{t.video2}:</span>
                                                        <span className="font-medium">{t.uploaded}</span>
                                                    </div>
                                                )}
                                                {formData.video && (
                                                    <div className="flex justify-between">
                                                        <span className="text-gray-600">{t.video}:</span>
                                                        <span className="font-medium">{t.uploaded}</span>
                                                    </div>
                                                )}
                                                {formData.docx && (
                                                ...
                                                {formData.xlsx && (
                                                    <div className="flex justify-between">
                                                        <span className="text-gray-600">{t.excelFile}:</span>
                                                        <span className="font-medium">{t.uploaded}</span>
                                                    </div>
                                                )}
                                                <div className="flex justify-between">
                                                ...
                                                <div className="flex justify-between">
                                                    <span className="text-gray-600">{t.cubicMeters}:</span>
                                                    <span className="font-medium">{formData.cubic_metr ? t.yes : t.no}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="flex gap-4">
                        ...
                        {currentStep < totalSteps ? (
                            <button
                                ...
                                disabled={!canProceedToNextStep() || isGenerating}
                            >
                                {steps[currentStep - 1]?.optional ? t.skip : t.next}
                            </button>
                        ) : (
                            <button
                                onClick={generateReport}
                                ...
                                disabled={isGenerating}
                            >
                                {isGenerating ? t.generating : t.createReportBtn}
                            </button>
                        )}
                    </div>
                ...
                <div className="fixed inset-0 bg-[#00000017] bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl p-6 max-w-sm w-full mx-4 shadow-xl">
                        <div className="text-center">
                            ...
                            >
                                {notification.type === 'success' ? t.goToHistory : t.close}
                            </button>
                        </div>
                    </div>
                </div>
...
function FileUploadArea({ type, accept, onFileSelect, selectedFile, optional = false, maxSize, title, dataType, t }) {
    // Добавить новые типы в getIcon()
    const getIcon = () => {
        switch (type) {
            case 'image':
    ...
    const getTitle = () => {
        ...
        switch (type) {
            ...
            case 'video':
                return t.selectVideo
            case 'docx':
                return t.selectDocx
            case 'xlsx':
                return t.selectXlsx
            case 'data':
                return dataType === 'qogi' ? t.selectCsv : t.selectXlsxData
            default:
                return t.selectFile
        }
    }
    ...
    const getDescription = () => {
        if (selectedFile) return t.fileUploaded

        let desc = t.clickToSelect
        if (maxSize) desc += ` (${interpolateString(t.maxSize, { size: maxSize })})`
        if (optional) desc += ` - ${t.optional}`
    ...
    const getFileFormat = () => {
        switch (type) {
            case 'image':
                return t.jpgPngGif
            case 'video':
                return t.onlyTsFiles
            case 'docx':
                return t.onlyDocxFiles
            case 'xlsx':
                return t.onlyXlsxFiles
            case 'data':
                return dataType === 'qogi' ? t.csvFiles : t.xlsxFiles
            default:
                return ''
        }
    }
    ...
    return (
        <div className="border-2 border-dashed border-[#006045] rounded-xl p-8 text-center hover:border-emerald-400 transition-colors">
            ...
            <label htmlFor={`file-${type}-${Date.now()}`} className="cursor-pointer">
                ...
                {getFileFormat() && (
                    <div className="text-xs text-gray-500 mb-4">
                        {interpolateString(t.supportedFormats, { formats: getFileFormat() })}
                    </div>
                )}
                {selectedFile ? (
                    <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-emerald-100 text-emerald-800">
